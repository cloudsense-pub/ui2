...
-- library init
	...
	local themes = {
		preset = {
			...
			["inline"] = hex("#2A1018"), --
			["accent"] = hex("#FF4DA6"), --
			["high_contrast"] = hex("#16080F"),
			["low_contrast"] = hex("#1C0B12"),
			["text"] = hex("#F2D7E6"),
...
-- library functions 
	...
	-- elements 
		...
		function library:confirmation(properties)
			local cfg = {
				title = properties.title or properties.name or "Confirmation",
				text = properties.text or "Are you sure?",
				callback = properties.callback or function() end,
			}

			if flags["disable_confirmations"] == true then
				cfg.callback(true)
				return
			end
			...
			local outline = library:create("Frame", {
				...
				BorderColor3 = rgb(0,0,0),
				BorderSizePixel = 0,
				Size = dim2(0, 260, 0, 130),
				Position = dim2(0.5, -130, 0.5, -65),
				BackgroundColor3 = themes.preset.outline,
				ZIndex = 50,
			})
			library:apply_theme(outline, "outline", "BackgroundColor3")
			...
			library:apply_theme(msg, "text", "TextColor3")

			local function makeBtn(text, x)
				local b = library:create("TextButton", {
					Parent = bg,
					BorderSizePixel = 0,
					Position = dim2(0, x, 0, 62),
					Size = dim2(0, 110, 0, 22),
					BackgroundColor3 = themes.preset.outline,
					Text = "",
					AutoButtonColor = false,
					ZIndex = 53,
				})
				library:apply_theme(b, "outline", "BackgroundColor3")
			...
			local confirmBtn = makeBtn("Confirm", 16)
			local cancelBtn = makeBtn("Cancel", 134)

			local disable = library:create("TextButton", {
				Parent = bg,
				BackgroundTransparency = 1,
				BorderSizePixel = 0,
				Position = dim2(0, 16, 0, 95),
				Size = dim2(1, -32, 0, 16),
				Text = "",
				ZIndex = 53,
			})
			...
			library:apply_theme(boxFill, "accent", "BackgroundColor3")
			local dlab = library:create("TextLabel", {
				Parent = disable,
				BackgroundTransparency = 1,
				Position = dim2(0, 18, 0, 0),
				Size = dim2(1, -18, 1, 0),
				FontFace = library.font,
				Text = "Disable Confirmations?",
				TextColor3 = themes.preset.text,
				TextSize = 12,
				TextXAlignment = Enum.TextXAlignment.Left,
				ZIndex = 54,
			})
			library:apply_theme(dlab, "text", "TextColor3")

			local disabled = false
			local function finish(ok)
				pcall(function() outline:Destroy() end)
				if ok and disabled then
					flags["disable_confirmations"] = true
				end
				cfg.callback(ok == true)
			end

			disable.MouseButton1Click:Connect(function()
				disabled = not disabled
				boxFill.Visible = disabled
			end)
